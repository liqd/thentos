...
runThentosUpdate :: ThentosUpdate DB ThentosError a -> Update DB (Either ThentosError a)
...
runThentosQuery :: ThentosQuery DB ThentosError a -> Query DB (Either ThentosError a)
...
polyUpdate :: forall a db e . AsDB db e => ThentosUpdate DB ThentosError a -> ThentosUpdate db e a
polyUpdate upd = EitherT . StateT $ Identity . (asDB (Proxy :: Proxy (db, e)) %%~ bare)
  where
    bare :: DB -> (Either e a, DB)
    bare = runIdentity . runStateT (fmapL (asDBError (Proxy :: Proxy (db, e))) <$> runEitherT upd)

-- | Turn a query transaction on 'DB' into one on any 'AsDB' instance.  See also 'polyUpdate'.
polyQuery :: forall a db e . AsDB db e => ThentosQuery DB ThentosError a -> ThentosQuery db e a
polyQuery qry = EitherT . ReaderT $ \ (state :: db) ->
    fmapL (asDBError (Proxy :: Proxy (db, e))) <$> runEitherT qry `runReaderT` (state ^. (asDB (Proxy :: Proxy (db, e))))
