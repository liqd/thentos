...
newtype Action db a = Action { fromAction :: ReaderT (ActionState db) (EitherT (ThentosError db) (LIO DCLabel)) a }
  deriving ( Functor
           , Applicative
           , Monad
...
update'P e = do
    ...
    result :: EventResult event <- liftLIO . ioTCB $ update' st e
    either (throwError . asDBActionError) return result
